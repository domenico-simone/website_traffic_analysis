...
if __name__ == "__main__":
    ...
    try:
        ...
        # console_logger.info(f"Daily report for {log_date} written in file ")
        # #
        # out_folder_stats = f"data/stats/daily/{args.grouping_field}"
        # out_file = os.path.join(out_folder_stats, f"{os.path.basename(log_date)}_stats.csv")
        # os.makedirs(out_folder_stats, exist_ok=True)
        # daily_stats.coalesce(1).write.csv(out_file, header=True, mode="overwrite")
        # # rename part_file generated by write.csv and rearrange directories
        # part_file = os.path.join(out_file, "*csv")
        # os.rename(part_file, out_file)
        # os.rmdir(out_file + ".csv")
        # #
